...
export function createDevignServer(config: ServerConfig) {
  ...
  server.resource(
    "current-page",
    "figma://current-page",
    { description: "Current Figma page structure (node tree)", mimeType: "application/json" },
    async () => {
      if (!bridge.isConnected()) {
  ...
  server.resource(
    "selection",
    "figma://selection",
    { description: "Currently selected nodes in Figma", mimeType: "application/json" },
    async () => {
      if (!bridge.isConnected()) {
  ...
  server.resource(
    "styles",
    "figma://styles",
    { description: "Local paint, text, effect, and grid styles", mimeType: "application/json" },
    async () => {
      if (!bridge.isConnected()) {
  ...
  server.resource(
    "variables",
    "figma://variables",
    { description: "Local variable collections and design tokens", mimeType: "application/json" },
    async () => {
      if (!bridge.isConnected()) {
  ...
  server.resource(
    "components",
    "figma://components",
    { description: "Components available on the current page", mimeType: "application/json" },
    async () => {
      if (!bridge.isConnected()) {
  ...
  server.prompt(
    "create-screen",
    "Guided workflow: create a full screen/page layout in Figma from a description",
    {
...
Create the design now.`,
  ...
  server.prompt(
    "build-design-system",
    "Set up a complete design system with variables, styles, and base components",
    {
...
Build this design system now.`,
  ...
  server.prompt(
    "audit-accessibility",
    "Audit the current page for accessibility issues (contrast, text size, touch targets)",
    {},
    async () => ({
      messages: [{
        role: "user",
